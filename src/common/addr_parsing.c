...
char *resolve_one_name(const char *orig_str, char *str_p)
{
  ...
  int pos = 0, len = BUF_SIZE;
  char *new_str = (char*) malloc(len);
  if (!new_str) {
    goto out;
  }

  firstcolon = strchr(str, ':');
  lastcolon = strrchr(str, ':');
  bracecolon = strstr(str, "]:");
  ...
  r = getaddrinfo(str, port_str, &hint, &res);
  if (r < 0) {
...
char *resolve_addrs(const char *orig_str)
{
  ...
  while (tok) {
    ...
    char *res = resolve_one_name(tok, tok);
    if (!res) {
      free(new_str);
      free(buf);
      return NULL;
    ...
    pos = safe_cat(&new_str, &len, pos, res);
